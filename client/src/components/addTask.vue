<template>
  <a-modal v-model="showModal" :maskClosable="false" :footer="false" :class="'header-not-border'"
    :bodyStyle="{ padding: '0 32px 32px 32px' }" :width="680" class="add-address-modal">
    ...
    <div class="add-address">
      <div class="input-content">
        ...
        <a-input v-model="name" placeholder="Task name" :class="{ 'validate-err': validateName }" />
      ...
      <div class="input-content">
        ...
        <a-input v-model="time" placeholder="Estimate time" :class="{ 'validate-err': validateTime }" />
        <div class="box-validate-pos" v-if="validateTime">
          <span class="text-validate">Estimate time cannot be left blank!</span>
        </div>
      </div>
      <div class="input-content">
        <span class="input-title">Assign task to<span style="color: red">*</span></span>
        <select style="
            height: 32px;
            border-radius: 4px;
            padding-left: 10px;
            border: 1px solid #d9d9d9;
          " v-model="assign" placeholder="User" :class="{ 'validate-err': validateAssign }">
          <option v-for="user in users" :key="user._id" :value="user._id">
            {{ user.name }}
          </option>
        </select>
        <div class="box-validate-pos" v-if="validateAssign">
          <span class="text-validate">Assign task to cannot be left blank!</span>
        </div>
      </div>
      <div class="input-content">
        <span class="input-title">Priority<span style="color: red">*</span></span>
        <select style="
            height: 32px;
            border-radius: 4px;
            padding-left: 10px;
            border: 1px solid #d9d9d9;
          " v-model="priority" placeholder="Priority" :class="{ 'validate-err': validatePriority }">
      ...
      <div class="input-content">
        ...
        <a-textarea v-model="description" placeholder="Description" :class="{ 'validate-err': validateDescription }" />
      ...
      <div class="add-shop-footer">
        <a-button @click="hide" style="width: 73px; border-radius: 8px; height: 37px">
          Cancel
        </a-button>
        <a-button @click="createClick" type="primary" style="
            width: 73px;
            margin-left: 8px;
            border-radius: 8px;
            height: 37px;
